import { ServiceXmlhttprequest } from "./serviceXmlhttprequest.js";

const service = new ServiceXmlhttprequest();
...
function buttonLoad() {
    ...
    service.getDataForForecast(chosen_city, ls => {
        ls.forEach(data => loadForecastData(data))
    });

    service.getDataForCity(chosen_city, ls => loadTempData(ls));

    service.getDataForCity(chosen_city, ls => loadTotalPrecipitationData(ls));

    service.getDataForCity(chosen_city, ls => loadAverageWindData(ls));

    service.getDataForCity(chosen_city, ls => latestMeach(ls));
}
//Test
